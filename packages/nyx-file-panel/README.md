...
| Edit happens in place | Editing a message no longer creates anything: the host appends the edited text as a **replacement** of the surface range that starts at that message and runs to the end of the conversation (the same mechanism DSH's own compaction uses), so the message is rewritten where it was and everything after it leaves the conversation. DSH's own prompt path then sends the text, and the copy that path always appends is folded away again (`edit-settle`). Nothing is deleted from the log — ranges are shadowed — and no session is created. Needs the host half (restart once after installing). |
| Message versions (`‹ n/m ›`) | Every edit keeps a family: under the edited message a small `‹ 2/2 ›` ring switches between the versions — `‹` goes back to the original, `›` forward to the newest branch. Each version is a session version (DSH has no branching inside one session, and its own *Branch* action creates a session too), so the conversation reads as one thread while the original stays untouched. |
