...
| Path trong tin nhắn | Path chỉ là chữ — trong `code` hay trong câu văn — cũng bấm được. Plugin đọc đúng token ngay dưới con trỏ, ghép với workspace root nếu là path tương đối, mở `~` thành home, rồi đưa qua đúng cái cổng kiểm tra tồn tại. Path thật thì có gạch chân chấm; `and/or`, `24/7` thì không. Bấm kèm phím tắt thì plugin không đụng vào. |
| Turn navigator | Dải "nhảy theo lượt" 28px của DSH nằm đúng chỗ panel cần, nên panel cho nó nghỉ: `window.__dshFilePanel.debug.hideTurnRail(false)` là nó trở lại. |
